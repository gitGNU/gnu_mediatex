/*=======================================================================
 * Version: $Id: md5sum.c,v 1.9 2015/09/17 18:53:48 nroche Exp $
 * Project: MediaTeX
 * Module : checksums
 *
 * md5sum computation
...
/*  =======================================================================*\/ */
/* static int  */
/* computeQuickMd5(int fd, ssize_t *sum, MD5_CTX *md5Ctx,  */
/* 		off_t size, char quickMd5sum[MAX_SIZE_MD5 + 1]) */
/* { */
/*   int rc = FALSE; */
/*   char buf[512]; */
/*   ssize_t bytes; */
...
/*   memcpy(&copy, md5Ctx, sizeof(MD5_CTX)); */
...
/*  *                          off_t size, char fullMd5sum[MAX_SIZE_MD5+1]) */

/*  * Input      : int fd: file descriptor to use for computation */
/*  *              ssize_t *sum: off_t size: maximum size of the file  */
/*  *                 (for progbar) */
/*  *              MD5_CTX *md5Ctx: md5sum data at the end of quick computation */
/*  *              off_t size: size where we continue computation */
/*  * Output     : char fullMd5sum[MAX_SIZE_MD5 + 1]): the resuling md5sum */
/*  * Note       : On GNU/Linux there is a bug with last block on CDROM */
/*  *              so we truncate reading using size parameter */
/*  =======================================================================*\/ */
/* static int  */
/* computeFullMd5(int fd, ssize_t *sum, MD5_CTX *md5Ctx,  */
/* 	       off_t size, char fullMd5sum[MAX_SIZE_MD5 + 1]) */
/* { */
/*   int rc = FALSE; */
/*   char buf[512]; */
...
/*   char quickSha1sum[MAX_SIZE_SHA1+1]; */
...
/*   strncpy(quickSha1sum, data->quickSha1sum, MAX_SIZE_SHA1+1);  */
/*   strncpy(fullSha1sum, data->fullSha1sum, MAX_SIZE_SHA1+1);  */
...
/*   switch (data->opp) { */
/*   case MD5_CACHE_ID: */
/*     rc = computeQuickMd5(fd, &sum, &md5Ctx, data->size, data->quickMd5sum); */
/*     rc&= computeFullMd5(fd, &sum, &md5Ctx, data->size, data->fullMd5sum); */
/*     break; */
/*   case MD5_SUPP_ADD: */
/*     MD5_Init(&md5Ctx); */
/*     rc = computeQuickMd5(fd, &sum, &md5Ctx, data->size, data->quickMd5sum); */
